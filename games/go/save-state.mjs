...
export function validateGoSave(value) {
  ...
  return {
    ...
    running: Boolean(value.running), history: value.history.map((item) => ({ ...item, board: item.board.map((row) => [...row]), captures: { ...item.captures }, lastMove: item.lastMove && { ...item.lastMove } })),
    ...
    settingsOpen: Boolean(value.settingsOpen), scoring: Boolean(value.scoring), deadStones: [...value.deadStones],
    scoringConfirmations: [...new Set(value.scoringConfirmations)], draft, resultOpen: Boolean(value.resultOpen),
